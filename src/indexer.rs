        ...
        { 
            ...
            else 
            {
                if word.len() > 0
                {
                    ...
                    r = word_hash::hash_word_to_u1282(&word);
    ...
    fn parse_file2(doc_id: u32, file_name: &str, content: &mut String, hm:&mut HashMap<u128,WordBlock>, com_words:& HashMap<u128, u8>) ->  io::Result<u32> 
    {
        {
            //println!("{}", file_name);
            let mut file = File::open(file_name)?;
    ...
    */
    ...
    fn index(doc_files: &Vec<String>, hm: &mut HashMap<u128,WordBlock>, source_path:&str, common_word_path:&str,collection_index:u32,collection_count: u32, worker_id:u8, worker_count:u8, limit:u32)
    {
        //let full_path = Path::new(source_path).join(doc_collection.to_string()).display().to_string();
        ...
        for doc_file in docs_to_process 
        {
            ...
            parse_file2(doc_id, &doc_file, &mut content, hm, &com).expect("Unable to parse file.");
            count = count + 1;
    ...
    pub fn index_all(source_path:&'static str, common_word_path:&'static str, collection_count:u32, worker_count:u8, limit:u32)->io::Result<()>
    {
        ...
        for collection_index in 0..collection_count 
        {
            ...
            count += index_files(&doc_files_arc, source_path, common_word_path, collection_index, collection_count, worker_count, limit);
        }
    ...
    pub fn index_files(doc_files_arc: &Arc<Vec<String>>, source_path:&'static str, common_word_path:&'static str, collection_index:u32, collection_count: u32, worker_count:u8, limit:u32) -> u64
    {
        if worker_count == 1
        {
            let mut doc_files = Vec::new();
            ...
            let s = Instant::now();
            index(&doc_files,&mut hm, source_path,common_word_path,collection_index,collection_count,255,1,limit);
        ...
        else
        {
            ...
            for i in 0..worker_count 
            {
                ...
                workers.push(thread::spawn(move || {
                    ...
                    index(&clone_arc, &mut hm, source_path,common_word_path,collection_index,collection_count,i,worker_count,limit);
